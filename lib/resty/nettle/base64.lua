...
function encoder:single(src)
    local len = lib.nettle_base64_encode_single(self.context, types.char_16, byte(src))
    return ffi_str(types.char_16, len), tonumber(len)
end
...
function encoder:final()
    local len = lib.nettle_base64_encode_final(self.context, types.char_24)
    return ffi_str(types.char_24, len), tonumber(len)
end
...
function decoder:single(src)
    local len = lib.nettle_base64_decode_single(self.context, types.uint8_t_8, byte(src))
...
function base64.encode(src, urlsafe)
    ...
    local fnl = lib.nettle_base64_encode_final(ctx, types.char_24)
    if fnl > 0 then
        return dst .. ffi_str(types.char_24, fnl)
    end
    return dst
end
